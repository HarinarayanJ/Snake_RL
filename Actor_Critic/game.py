...
def ai_game_loop():
    ...
    params = pickle.load(open("actor_critic_params.pkl", "rb"))
    ...
    while True:
        ...
        draw_snake(snake)
        draw_food(food)
        draw_text(f"Actor-Critic Score: {score}", font, BLUE, 10, 10, center=False)
        pygame.display.flip()
...
def start_screen():
    ...
    while True:
        screen.fill(BLACK)
        draw_text("SNAKE ACTOR-CRITIC", title_font, BLUE, WIDTH // 2, 100)
...
def game_over_screen(score):
    ...
    while True:
        ...
        draw_text(f"High Score: {high}", font, BLUE, WIDTH // 2, 220)
        draw_text("Press ENTER to return",
                  pygame.font.SysFont("comicsans", 20),
                  WHITE, WIDTH // 2, 300)
        pygame.display.flip()

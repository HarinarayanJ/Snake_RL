...
def ai_game_loop():
    agent = SARSAAgent()
    agent.Q = pickle.load(open("sarsa_table.pkl", "rb"))
    ...
    while True:
        ...
        draw_snake(snake)
        draw_food(food)
        draw_text(f"SARSA Score: {score}", font, WHITE, 10, 10, center=False)
        pygame.display.flip()
...
def start_screen():
    ...
    while True:
        screen.fill(BLACK)
        draw_text("SNAKE SARSA", title_font, GREEN, WIDTH // 2, 100)
